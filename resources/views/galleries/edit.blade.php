...
<head>
    ...
    <title>Edit Gallery - Photo Platform</title>
        ...
        <main class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
                <div class="mb-6">
                    ...
                    <p class="mt-2 text-gray-600">Update your gallery settings</p>
                </div>
                ...
                <form method="POST" action="{{ route('galleries.update', $gallery) }}">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="space-y-2">
                            <label class="flex items-center">
                                <input type="radio" name="access_type" value="public" {{ old('access_type', $gallery->access_type) === 'public' ? 'checked' : '' }} class="mr-3">
                                <span class="text-sm text-gray-700">Public - Anyone with the link can view</span>
                            </label>
                            <label class="flex items-center">
                                <input type="radio" name="access_type" value="private" {{ old('access_type', $gallery->access_type) === 'private' ? 'checked' : '' }} class="mr-3">
                                <span class="text-sm text-gray-700">Private - Only you can view</span>
                            </label>
                            <label class="flex items-center">
                                <input type="radio" name="access_type" value="password_protected" {{ old('access_type', $gallery->access_type) === 'password_protected' ? 'checked' : '' }} class="mr-3" id="access_type_password">
                    ...
                    <div class="mb-6" id="password_field" style="display: {{ old('access_type', $gallery->access_type) === 'password_protected' ? 'block' : 'none' }};">
                    ...
                    <div class="mb-6">
                        ...
                        <input 
                            ...
                            id="expires_at" 
                            name="expires_at" 
                            value="{{ old('expires_at', $gallery->expires_at ? $gallery->expires_at->format('Y-m-d') : '') }}"
                    ...
                    <div class="flex justify-end space-x-4">
                        <a href="{{ route('galleries.show', $gallery) }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">
                            Cancel
                        </a>
        ...
        accessTypeRadios.forEach(radio => {
            radio.addEventListener('change', function() {
                if (this.value === 'password_protected') {
                    ...
                    // Only require if changing from non-password type
                    if ('{{ $gallery->access_type }}' !== 'password_protected') {
